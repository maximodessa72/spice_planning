    ...
    **Автоматические даты по неделям:**
    ...
    for group in st.session_state.groups:
        ...
        if procurement_current_mi < len(group_results):
            ...
            # Показываем ВСЕ приходы этого месяца — и зафиксированные вручную (in_transit),
            # и автоматически рассчитанные системой (is_auto_order). Раньше условие требовало
            # именно in_transit=True, из-за чего автоматические заказы сюда не попадали,
            # хотя физически товар всё равно приходит в этом месяце.
            if arrival_kg > 0:
