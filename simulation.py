...
def get_bottleneck_recommendations(group: Dict, results: List[Dict]) -> List[Dict]:
    ...
    заказов):
      - Если на сам месяц j+cycle_m уже стоит реальный приход (фикс./авто,
        размещённый заранее) — пропускаем: он и так решит вопрос, а низкий
        буфер "до прихода" в этот момент — это норма, не проблема.
      - Иначе смотрим на буфер позиций в месяце (j + cycle_m). Этот буфер
        (icb) уже учитывает любые промежуточные приходы между j и target_mi —
        он считается последовательно по факту балансов.
      - Если несмотря на всё это к моменту target_mi позиция всё равно
        ниже порога — значит промежуточных приходов было недостаточно,
        и нужна рекомендация. Считаем гипотетический заказ на балансах
        месяца target_mi (месяца дефицита) — так же, как балансируется
        реальный заказ.
    ...
    for j in range(len(results)):
        ...
        # На сам месяц дефицита УЖЕ есть реальный приход (фикс./авто, размещённый
        # заранее) — он и так закроет проблему, когда прибудет. То, что буфер
        # "до прихода" низкий — это нормально, он всегда низкий прямо перед
        # пополнением. Рекомендация тут не нужна.
        r_target = results[target_mi]
        if r_target["in_transit"] or r_target.get("is_auto_order", False) or r_target["arrive"] > 0:
            continue

        icb_target = r_target["icb"]  # буфер позиций к моменту дефицита
